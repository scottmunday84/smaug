...
/*    if (ch->trust != 0)*/
	...
	ch->pcdata->last_news,ch->pcdata->last_changes,ch->pcdata->last_claninfo);
    fprintf( fp, "Scro %d\n", 	ch->lines		);
    fprintf( fp, "Room %d\n",
	...
	fprintf( fp, "Reca %d\n",	ch->pcdata->recall	);
        ...
        fprintf( fp, "Alos %d\n",	ch->pcdata->alosses	);
        fprintf( fp, "Pkp  %d\n",	ch->pcdata->pkp		);
        fprintf( fp, "Kill2 %d %d %d %d\n", 
        	 ch->pcdata->kills[0],
		 ch->pcdata->kills[1],
		 ch->pcdata->kills[2],
		 ch->pcdata->kills[3]	);
        fprintf( fp, "Deth2 %d %d %d %d\n",
		 ch->pcdata->deaths[0],
		 ch->pcdata->deaths[1],
		 ch->pcdata->deaths[2],
		 ch->pcdata->deaths[3]	);
	...
	fprintf( fp, "Affc2 '%s' %3d %3d %3d %3d %3d %10d %6d\n",
...
void fwrite_obj( CHAR_DATA *ch, OBJ_DATA *obj, FILE *fp, int iNest )
{
    ...
    if(!IS_IMMORTAL(ch))
    {
     if ( (ch->level < obj->level - 10 && obj->level > 30
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    for(stat=0;stat<(MAX_GS+1);stat++)
    {
     ch->pcdata->kills[stat]		= 0;
     ch->pcdata->deaths[stat]		= 0;
    }
	...
	int i;
        ...
        if (ch->race == 0 || !race_table[ch->race].pc_race)
	    ch->race = race_lookup("sivah");

	ch->size = pc_race_table[ch->race].size;
        /* if the char is affected by anything changing his size,
           we need to fix that right now because affects have
           already been added in fread_char - Fade, 2001 */
        for(paf=ch->affected;paf!=NULL;paf=paf->next)
        {
         if((paf->location==APPLY_SIZE) && (ch->size!=paf->modifier))
         { 
           ch->size=paf->modifier;
           break;
         }
        }
...
bool load_char_reroll( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->comm				= COMM_COMBINE 
					| COMM_PROMPT
					| COMM_STORE;
    ch->prompt 				= str_dup("<%hhp %mm %vmv{x> ");
    ch->pcdata->confirm_delete		= FALSE;
    ch->pcdata->pwd			= str_dup( "" );
    ch->pcdata->bamfin			= str_dup( "" );
    ch->pcdata->bamfout			= str_dup( "" );
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'A':
		...
		break;
            ...
            if (!str_cmp(word, "Affc"))
            {
                ...
                sn = skill_lookup_exact(fread_word(fp));
                if (sn < 0)
                    bug("Fread_char: unknown skill.",0);
                else
                    paf->type = sn;
	...
	case 'B':
	    ...
	    KEY( "Boun",	ch->pcdata->bounty,	fread_number( fp ) );
            KEY( "Bala",	ch->balance,		fread_number( fp ) );
	    break;
	...
	case 'D':
	    ...
	    KEY( "Desc",	ch->description,	fread_string( fp ) );
            KEY( "Deth",	dummy,			fread_number( fp ) );
	    if (!str_cmp( word, "Dupes"))
            ...
            if (!str_cmp(word,"Deth2"))
	    {
		ch->pcdata->deaths[0] = fread_number( fp );
		ch->pcdata->deaths[1] = fread_number( fp );
		ch->pcdata->deaths[2] = fread_number( fp );
		ch->pcdata->deaths[3] = fread_number( fp );
		fMatch = TRUE;
		break;
	    }
	    break;
	...
	case 'E':
	    ...
	    KEY( "Exp",		ch->exp,		fread_number( fp ) );
            KEY( "Exb1",	ch->exbit1,		fread_flag(fp));
	    break;
	...
	case 'I':
	    ...
	    break;
        case 'K':
            KEY( "Kfla", 	ch->pcdata->kflag,	fread_number( fp ) );
            KEY( "Kill",	dummy,			fread_number( fp ) );
            if (!str_cmp(word,"Kill2"))
	    {
		ch->pcdata->kills[0] = fread_number( fp );
		ch->pcdata->kills[1] = fread_number( fp );
		ch->pcdata->kills[2] = fread_number( fp );
		ch->pcdata->kills[3] = fread_number( fp );
		fMatch = TRUE;
		break;
	    }
            break;	
	...
	case 'N':
		...
		ch->pcdata->last_news			= fread_number(fp);
		ch->pcdata->last_changes		= fread_number(fp);
		ch->pcdata->last_claninfo		= fread_number(fp);
		fMatch = TRUE;
		break;
	    }
	    break;
	...
	case 'P':
	    ...
	    KEY( "Prac",	ch->practice,		fread_number( fp ) );
 	    ...
 	    KEY( "Prom",	ch->prompt,		fread_string( fp ) ); 
            KEY( "Pkp",		ch->pcdata->pkp,	fread_number( fp ) );
	    break;
	...
	case 'S':
            KEY( "Stan",	ch->pcdata->stance,	fread_number( fp ) );
		...
		/* sn    = skill_lookup( fread_word( fp ) ); */
                /* the following code is necessary to transform
                   old skills/spells into their new version */
                if(!strcmp(temp,"armor"))
                 sn = skill_lookup("immaterial armor");
                else if(!strcmp(temp,"shield"))
                 sn = skill_lookup("force shield");
                else if(!strcmp(temp,"scrolls"))
                 sn = skill_lookup("thaumaturgy");
                else if(!strcmp(temp,"create food"))
                 sn = skill_lookup("nourish");
		else if(!strcmp(temp,"giant strength"))
                 sn = skill_lookup("vigor");
		if ( sn < 0 )
		{
		    fprintf(stderr,"%s",temp);
		    bug( "Fread_char: unknown skill. ", 0 );
	...
	case 'V':
		...
		break;
            ...
            if (!str_cmp( word, "View"))
            {
                ...
                ch->pcdata->view[vcount]        = fread_string(fp);
                vcount++;
                fMatch = TRUE;
            }
	    break;
...
void fread_obj( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'C':
	    ...
	    KEY( "Cost",	obj->cost,		fread_number( fp ) );
            KEY( "Clan",	obj->clan,		fread_number( fp ) );
	    KEY( "Clas",	obj->class,		fread_number( fp ) );
	    break;
