...
void do_gain(CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf, "%-17s %-5s %-17s %-5s %-17s %-5s\n\r",
	...
	col = 0;
        ...
        sprintf(buf, "%-5s %-15s %-5s %-5s %-15s %-5s %-5s %-15s %-5s\n\r",
                     "level","skill","cost","level","skill","cost","level","skill","cost");
        send_to_char(buf,ch);
...
void do_spells(CHAR_DATA *ch, char *argument)
{
	...
	if (ch->level < lev)
	  sprintf(buf,"{x%-16s  {cn/a{x           ", skill_table[sn].name);
	else
	{
	  mana = UMAX(skill_table[sn].min_mana,
		      100/(2 + ch->level - lev));
	  sprintf(buf,"{m%-16s  {c%3d mana{m %3d%%{x ",skill_table[sn].name,mana,
           ch->pcdata->learned[sn]);
	}
	
	if (spell_list[lev-1][0] == '\0')
	  sprintf(spell_list[lev-1],"\n\r{CLevel %3d: %s{x",lev,buf);
        else /* append */
	{
	  if ( ++spell_columns[lev-1] % 2 == 0)
            strcat(spell_list[lev-1],"\n\r           ");
	  strcat(spell_list[lev-1],buf);
        }
      }
    }
...
void do_skills(CHAR_DATA *ch, char *argument)
{
    ...
    for (sn = 0; sn < MAX_SKILL; sn++)
    {
	  ...
	  ch->pcdata->learned[sn] > 0)
      {
        ...
        lev = skill_table[sn].skill_level[ch->class];
        if (ch->level < lev)
          sprintf(buf,"{x%-18s {cn/a{x      ", skill_table[sn].name);
        else
          sprintf(buf,"{m%-18s {c%3d%%{x      ",skill_table[sn].name,
					 ch->pcdata->learned[sn]);
 
        if (skill_list[lev-1][0] == '\0')
          sprintf(skill_list[lev-1],"\n\r{CLevel %3d: %s",lev,buf);
        else /* append */
        {
          if ( ++skill_columns[lev-1] % 2 == 0)
            strcat(skill_list[lev-1],"\n\r           ");
          strcat(skill_list[lev-1],buf);
        }
      }
    }
...
void list_group_costs(CHAR_DATA *ch)
{
	...
	&&  group_table[gn].rating[ch->class] > 0)
	{
	    sprintf(buf,"%-17s %-5d ",group_table[gn].name,
    ...
    sprintf(buf,"%-5s %-15s %-5s %-5s %-15s %-5s %-5s %-15s %-5s\n\r","level","skill"," cp","level","skill"," cp","level","skill"," cp");
    send_to_char(buf,ch);
    ...
    sprintf(buf,"{cCreation points: {D%d{x\n\r",ch->pcdata->points);
    send_to_char(buf,ch);
    sprintf(buf,"{cExperience per level: {D%ld{x\n\r",
...
void list_group_chosen(CHAR_DATA *ch)
{
    ...
    sprintf(buf,"%-18s %-5s %-18s %-5s %-18s %-5s","group","cost","group","cost","group","cost\n\r");
    send_to_char(buf,ch);
    ...
    sprintf(buf,"%-18s %-5s %-18s %-5s %-18s %-5s","skill","cost","skill","cost","skill","cp\n\r");
    send_to_char(buf,ch);
...
void do_class(CHAR_DATA *ch, char *argument)
{
	...
	send_to_char( "{mGroups you currently have:\n\r", ch );
	send_to_char( "{c--------------------------{x\n\r", ch );
	for (gn = 0; gn < MAX_GROUP; gn++)
        {
	    if (group_table[gn].name == NULL)
	    ...
	    if (ch->pcdata->group_known[gn])
	    {
		sprintf(buf,"{m%-20s{x ",group_table[gn].name);
		send_to_char(buf,ch);
		if (++col % 3 == 0)
		    send_to_char("\n\r",ch);
	    }
	...
	return;
     ...
     if (!str_cmp(argument,"all"))    /* show all groups */
     {
	send_to_char( "{mGroups available to your class:\n\r", ch );
	send_to_char( "{c-------------------------------{x\n\r", ch );
        for (gn = 0; gn < MAX_GROUP; gn++)
        {
            if (group_table[gn].name == NULL)
    ...
    send_to_char( "{mSpells available in this group:\n\r", ch );
    send_to_char( "{c-------------------------------{x\n\r", ch );
    send_to_char( "{CL{cevel{c-{MS{mpell{c--------------- {CL{cevel{c-{MS{mpell{c---------------{x\n\r", ch );
    for (sn = 0; sn < MAX_IN_GROUP; sn++)
    {
	if (group_table[gn].spells[sn] == NULL)
	    break;
...
void check_improve( CHAR_DATA *ch, int sn, bool success, int multiplier )
{
    ...
    {
 	multiplier=6;     
        bug("check_improve: multiplier was zero",0);
    }
