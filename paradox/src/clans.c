...
*/
	...
	TRUE,	FALSE,	"Used" },
     ...
     {"loner",		" {RL{Do{rn{De{Rr{x ",
	ROOM_VNUM_ALTAR,	ROOM_VNUM_ALTAR,
	OBJ_VNUM_PIT,
	TRUE,	TRUE,	"Unused" },

    {"Paradox",		"{GP{ga{Wr{Da{Wd{go{GX",
	ROOM_VNUM_ALTAR,	ROOM_VNUM_ALTAR,
	OBJ_VNUM_PIT,
	FALSE,	TRUE,	"ParadoX MUD - Immortal Staff"},

    {"The Twelve",		"{YT{yh{We{xT{Dw{xe{Wl{yv{Ye",
        9800,        9800,
        9803,
        FALSE,   TRUE,   "The Twelve - Fade's Elite" },

    {"ShadowBane",		"{rS{Dh{xa{rd{Wo{Dw{rB{Da{xn{re",
        9810,        9810,
        9811,
        FALSE,   TRUE,   "ShadowBane" },
...
void do_guild( CHAR_DATA *ch, char *argument )
{
	...
	if(!IS_IMMORTAL(victim))
        register_rank(victim->name,victim->pcdata->pkp,
		victim->race,victim->class,victim->clan);
	return;
    }
    ...
    victim->clan = clan;
    if(!IS_IMMORTAL(victim))
    register_rank(victim->name,victim->pcdata->pkp,victim->race,
		victim->class,victim->clan);
}
...
void do_member( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp(arg1, "accept"))
	{
	    ...
	    update_clanlist(ch, ch->invited, TRUE, FALSE);
            if(!IS_IMMORTAL(ch))
	    register_rank(ch->name,ch->pcdata->pkp,ch->race,ch->class,	
			ch->clan);
	    ch->invited = 0;
	    return;
	}
    ...
    if ( (victim->class < MAX_CLASS/2) && ( victim->level < 21 || victim->level > 101 ) )
    {
	send_to_char( "They must be between levels 21 -> 101.\n\r",ch);
	return;
    }
    if ( (victim->class >= MAX_CLASS/2) && ( victim->level < 21 || victim->level > 101 ) )
    {
...
void do_clantalk( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
      ...
      sprintf( buf, "{x[{x%s{x] {cYou{D: {C%s{x\n\r",
       clan_table[ch->clan].who_name,
       argument );
      send_to_char( buf, ch );
      sprintf( buf, "{x[{x%s{x] {c$w{D: {C%s{x",
        clan_table[ch->clan].who_name,
           argument );
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
	...
	bool found = FALSE;
        ...
        if ( d->connected == CON_PLAYING &&
             d->character != ch &&
	     (is_same_clan(ch,d->character) || (IS_IMMORTAL(d->character)
                   && (get_trust(d->character)>=ENFORCER)))
             && 
