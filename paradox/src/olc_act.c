...
AEDIT( aedit_file )
{
    ...
    {
     if(!strcmp(p->file_name,file))
     {
      send_to_char("This file name is already in use.\n\r",ch);
      return FALSE;
     }
    }
...
AEDIT( aedit_security )
{
    ...
    if ( value > ch->pcdata->security || value < 1 )
    {
	if ( ch->pcdata->security != 0 )
	{
	    sprintf( buf, "Security is 1-%d.\n\r", ch->pcdata->security );
	    send_to_char( buf, ch );
	}
	else
	    send_to_char( "Security is 1 only.\n\r", ch );
	return FALSE;
    }
...
REDIT( redit_create )
{
    ...
    pRoom->area			= pArea;
    pRoom->vnum			= value;
    pRoom->name			= str_dup("Standard room");
...
OEDIT( oedit_show )
{
    ...
    if(pObj->class>=0)
     sprintf( buf,"Class:       [%s]\n\r",
        class_table[pObj->class].name);
    else
     sprintf( buf,"Class:       [none]\n\r");
    send_to_char( buf, ch);
...
OEDIT( oedit_create )
{
    ...
    pObj->vnum			= value;
    pObj->area			= pArea;
    pObj->name			= str_dup("standard object");
    pObj->short_descr		= str_dup("standard object");
    pObj->description		= str_dup("A standard object is lying here.");
...
MEDIT( medit_create )
{
    ...
    pMob->vnum			= value;
    pMob->area			= pArea;
    pMob->player_name		= str_dup("standard mob");
    pMob->short_descr		= str_dup("standard mob");
    pMob->long_descr		= str_dup("A standard mob is here.\n\r");
    pMob->description		= str_dup("A standard mob is here.\n\r");
    pMob->die_descr		= str_dup("");
    pMob->say_descr		= str_dup("");
...
MEDIT( medit_mdice )
{
    static char syntax[] = "Syntax:  mdice [<level>]\n\r";
    int level,ac;
    char clev[11],hitd[51],damd[51];
    MOB_INDEX_DATA *pMob;
    ...
    {
      level = pMob->level;
      send_to_char("Current mob level assumed as default.\n\r",ch);
    }
    else
     level = atoi(argument);
    if(level<1 || level > MAX_MOB_LEVEL)
    {
     send_to_char(syntax, ch);
     send_to_char("Level range is 1 to MAX_MOB_LEVEL.\n\r",ch);
     return FALSE;
    }
    strcpy(hitd,mdice_table[level-1].hitd);
    medit_hitdice(ch,hitd);
    strcpy(hitd,mdice_table[level-1].hitd);
    medit_manadice(ch,hitd);
    sprintf(clev,"%d",level);
    medit_hitroll(ch,clev);
    strcpy(damd,mdice_table[level-1].damd);
    medit_damdice(ch,damd);
    ac=mdice_table[level-1].ac;
    pMob->ac[AC_PIERCE] = ac;
    pMob->ac[AC_BASH]   = ac;
    pMob->ac[AC_SLASH]  = ac;
    pMob->ac[AC_EXOTIC] = ac;
    send_to_char( "Ac set.\n\r", ch );
    send_to_char( "Done.\n\r",ch);
    return TRUE;
}
...
void show_damlist(CHAR_DATA *ch)
{
    ...
    {
	if((attack_table[att].damage==DAM_MONK)
           || (att==40))
         continue;
        if ( ((att-1) % 21) == 0 )
	    add_buf(buffer,"{cName                 Noun{x\n\r");
