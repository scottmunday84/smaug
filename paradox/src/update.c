...
void	gs_update	args( ( bool oneleft ) );
...
/*    sprintf( buf, "the %s",
    ...
    add_mana 	= number_range((4*get_curr_stat(ch,STAT_INT)+get_curr_stat(ch,STAT_WIS))/4,(4*get_curr_stat(ch,STAT_INT)
...
int hit_gain( CHAR_DATA *ch )
{
	...
	if ( ch->pcdata->condition[COND_HUNGER]   > 0 )
	    gain += (gain*ch->pcdata->condition[COND_HUNGER])/96;

	if ( ch->pcdata->condition[COND_THIRST] > 0 )
	    gain += (gain*ch->pcdata->condition[COND_THIRST])/96;
...
void weather_update( void )
{
	...
	strcat( buf, "The {Ys{yu{Yn{x rises in the east.\n\r" );
	break;
	...
	{
	    strcat( buf, "The sky is getting {Wc{xl{Do{xu{Wd{Dy{x.\n\r" );
	...
	{
	    strcat( buf, "{YL{yi{Yg{Dh{xt{Yn{yin{Yg{x flashes in the sky.\n\r" );
...
{   
	...
	if ( IS_NPC(ch) && ch->hastimer )
	{
        ...
        if ( !IS_NPC(ch) )
        {
         if(ch->pcdata->kflag>0)
          ch->pcdata->kflag--;
         else if(ch->pcdata->kflag<0)
          ;
         else
...
void aggr_update( void )
{
	...
	for ( ch = wch->in_room->people; ch != NULL; ch = ch_next )
	{
		...
		&&   ( !IS_SET(ch->act, ACT_WIMPY) || !IS_AWAKE(vch) )
                &&   ( global_quest!=QUEST_CLOSED || !vch->on_quest ||
                       !IS_SET(quest_flags,QUEST_NOAGG))
		&&   can_see( ch, vch ) )
		{
...
void update_handler( void )
{
    ...
    static  int	    pulse_gs = (PULSE_TICK)+1;
    ...
    if (--pulse_gs <= 0)
    {
      pulse_gs = number_range(8,16) * (PULSE_TICK);
      gs_update(FALSE);
      if(game_state==GS_PARADOX)
       pulse_gs = number_range(3,5) * (PULSE_TICK);
    }
    else if (pulse_gs==PULSE_TICK)
      gs_update(TRUE);
...
void gs_update(bool oneleft)
{
 ...
 if(oneleft)
 {
  ...
  {
   if(d->connected==CON_PLAYING)
    send_to_char("{D<{GW{gorld {GI{gnfo{D>{x {GThe world tenses as something is about to change.{x\n\r",d->character);
  }
  reset_arena();
  arena = FIGHT_LOCK;
  gs_changing = TRUE;
 ...
 else
 {
  ...
  while(game_state == old_state)
   game_state = number_range(0,MAX_GS);
  switch(game_state) 
  {
   default:
    sprintf(buf2,"The world changes and everything becomes undefined.");
    break;
   case GS_CHAOS:
    sprintf(buf2,"{RT{rota{Rl {RC{rhao{Rs{G sweeps over the world!{x");
    break;
   case GS_SOLO:
    sprintf(buf2,"{GThe world switches into {YS{yolo {YM{yode{G.{x");
    break;
   case GS_1VS2:
    sprintf(buf2,"{GThe world switches into {R1{rvs{R2{G Mode.{x");
    break;
   case GS_PARADOX:
    sprintf(buf2,"{GThe world is consumed by {Rr{ra{Rg{re{G and {Yf{yu{Yr{yy{G - {GP{ga{Wr{Da{Wd{go{Gx is here!{x");
    break;
  }
  if(game_state!=GS_PARADOX)
   arena = FIGHT_OPEN;
  gs_changing = FALSE;
  sprintf(buf,"{D<{GW{gorld {GI{gnfo{D>{x %s\n\r",buf2);
  for(d=descriptor_list;d!=NULL;d=d->next)
  {
   if(d->connected==CON_PLAYING)
   {  
    send_to_char(buf,d->character);
    if(global_quest!=QUEST_CLOSED||!d->character->on_quest||
       !IS_SET(quest_flags,QUEST_COMBAT))
    {
...
{
 DESCRIPTOR_DATA *d;
 CHAR_DATA *chd=NULL,*ch=NULL;

 for(d=descriptor_list;d!=NULL;d=d->next)
 {
   if(d->connected==CON_PLAYING)
   { 
     if(IS_SET(d->character->exbit1,EXBIT1_CHALLENGER))
      ch=d->character;
     if(IS_SET(d->character->exbit1,EXBIT1_CHALLENGED))
      chd=d->character;
   }
 }
 if((chd==NULL) || (ch==NULL))
 ...
 if(ch->fighting!=NULL && (ch->fighting==chd))
  stop_fighting(ch,TRUE);
 if(chd->fighting!=NULL && (chd->fighting==ch))
  stop_fighting(chd,TRUE);
 if(IS_SET(ch->in_room->room_flags,ROOM_ARENA))
 {
  char_from_room(ch);
  char_to_room(ch,get_room_index(ROOM_VNUM_ALTAR));
 }
 if(IS_SET(chd->in_room->room_flags,ROOM_ARENA))
 {
  char_from_room(chd);
  char_to_room(chd,get_room_index(ROOM_VNUM_ALTAR));
 }
 do_look(ch,"auto");
 do_look(chd,"auto");
 REMOVE_BIT(ch->exbit1,EXBIT1_CHALLENGER);
 REMOVE_BIT(chd->exbit1,EXBIT1_CHALLENGED);
 REMOVE_BIT(ch->act,PLR_NORESTORE);
 REMOVE_BIT(chd->act,PLR_NORESTORE);
 ch->challenged=NULL; ch->challenger=NULL;
 chd->challenged=NULL; chd->challenger=NULL;
 ch->pcdata->pkflag=0;
 chd->pcdata->pkflag=0;
 arena = FIGHT_OPEN;
 comm_arena(NULL,NULL,"{WArena reset.");
}
