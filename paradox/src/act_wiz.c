...
void iquest_announce(char *argument)
{
 DESCRIPTOR_DATA *d;
...
void do_iquest( CHAR_DATA *ch, char *argument )
{
	...
	if (global_quest==QUEST_CLOSED)
        {
	    send_to_char("Too late, the quest is already closed.\n\r",ch);
	    return;
	}
    ...
    if (!str_cmp(arg1,"force"))
    {
	DESCRIPTOR_DATA *d;
        if(global_quest!=QUEST_CLOSED)
        {
	   send_to_char("A closed quest needs to be in progress in order to quest transfer.\n\r",ch);
           return;
        }
        for(d=descriptor_list; d!=NULL; d=d->next)
        {
         if(d->connected==CON_PLAYING && !IS_NPC(d->character) &&
            d->character->on_quest )
         {
          char buf[MAX_STRING_LENGTH];
          sprintf(buf,"%s %s %s %s",d->character->name,
              arg2,arg3,argument);
          do_force(ch,buf);
         }
        }
	...
	if(global_quest!=QUEST_CLOSED)
        {
	   send_to_char("A closed quest needs to be in progress in order to quest restore.\n\r",ch);
           return;
        }
        for (d = descriptor_list; d != NULL; d = d->next)
        {
	    ...
	    if (victim == NULL || IS_NPC(victim) || !victim->on_quest)
		continue;

            if ( IS_SET(victim->act, PLR_NORESTORE) )
            {
                act("$n attempts to quest restore you, but fails.",ch,NULL,victim,TO_VICT);
            } else {
		...
		if (victim->in_room != NULL)
		    act_new("$n has quest restored you.",ch,NULL,victim,TO_VICT,POS_DEAD);
	    }
        }
	send_to_char("All active questors restored.\n\r",ch);
	return;
    ...
    if (!str_cmp(arg1,"ban"))
    {
        ...
        if(global_quest!=QUEST_CLOSED)
        {
	   send_to_char("A closed quest needs to be in progress in order to ban a questor.\n\r",ch);
           return;
        }
        if(arg2[0]=='\0')
        {
	   send_to_char("Quest ban who?\n\r",ch);
           return;
        }
        if((qch = get_char_mortal(ch,arg2))==NULL)
        {
        ...
        if(!qch->on_quest)
        {
	   send_to_char("They haven't even joined the quest.\n\r",ch);
           return;
        }
        qch->on_quest = FALSE;
        send_to_char("You have been banned from the global quest.\n\r",qch);
        sprintf(buf,"{x[{YQ{yUES{YT{x]{c %s has been {Rb{ranne{Rd{c from the global quest.{x\n\r",capitalize(qch->name));
        iquest_announce(buf);
        return;
    }
    if(!str_cmp(arg1,"peace"))
    {
     ...
     if(global_quest!=QUEST_CLOSED)
        {
	   send_to_char("A closed quest needs to be in progress in order to use peace.\n\r",ch);
           return;
        }
      for ( d = descriptor_list; d != NULL; d = d->next )
      {
	...
	if(global_quest==QUEST_CLOSED)
	{
	    send_to_char("The quest has already been closed.\n\r",ch);
	    return;
	}
        if(global_quest==QUEST_OFF)
	{
	    send_to_char("How about starting a quest first (\"iquest on\")?\n\r",ch);
	    return;
	}
        global_quest=QUEST_CLOSED;
	...
	if (global_quest==QUEST_CLOSED)
	{
	    send_to_char("A closed quest is already in process.\n\r",ch);
	    return;
	}
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    if(!IS_NPC(victim))
    {
	sprintf(buf,"Suppress: %s\n\r",suppress_bit_name(victim->pcdata->suppress));
        add_buf(output,buf);
    }
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
 ...
 if (arg[0] == '\0') 
    ...
    if (is_number(arg))
    {
     ...
     sprintf(buf,"Paradox MUD will reboot in %i ticks.\n\r",reboot_counter);
     send_to_char(buf,ch);
     return;
    }
...
void reboot_rot( void )
{
    ...
    sprintf( log_buf, "Rebooting ParadoX MUD.");
    log_string(log_buf);
    for ( d = descriptor_list; d != NULL; d = d_next )
    {
	d_next = d->next;
	write_to_buffer(d,"ParadoX MUD is going down for rebooting NOW!",0);
        if (d->character != NULL)
...
/* FILE *fp;
 ...
 for(d=descriptor_list;d!=NULL;d=d_next)
 {
  ...
  {
    do_save(d->character,"");
    send_to_char("A voice whispers: 'It's all over now. Paradox MUD is crashing.'\n\r",d->character);
  }
  d_next=d->next;
  close_socket(d);
 }
 ...
 return;*/
	...
	/* For each playing descriptor, save its state */
        sprintf(buf,"\n\r** Paradox MUD is crashing...attempting copyover **\n\r");
...
void do_clone(CHAR_DATA *ch, char *argument )
{
	...
	wiznet("$N clones $p.",ch,clone,WIZ_LOAD,WIZ_SECURE,get_trust(ch));
        if(IS_OBJ_STAT(clone,ITEM_QUEST))
         printf_log("%s has cloned %s [Q] at room %d.",
...
void do_oload( CHAR_DATA *ch, char *argument )
{
    ...
    if(IS_OBJ_STAT(obj,ITEM_QUEST))
     printf_log("%s loads %s [Q] at room %d.",
      ch->name,obj->short_descr,ch->in_room->vnum);
    return;
}
...
void do_purge( CHAR_DATA *ch, char *argument )
{
	...
	for ( obj = ch->in_room->contents; obj != NULL; obj = obj_next )
	{
	    ...
	    if (!IS_OBJ_STAT(obj,ITEM_NOPURGE) && !IS_OBJ_STAT(obj,ITEM_QUEST))
	      extract_obj( obj );
	}
...
/*        int temp_prac;*/
	/* Forces the person to remove all the eq....  so level restriction still apply */
        for (obj = victim->carrying; obj; obj = obj_next)
        {
          ...
          if (obj->wear_loc != WEAR_NONE && can_see_obj (victim, obj))
          {
           obj_from_char(obj);
...
/*	victim->practice = temp_prac;*/
	...
	{
	    update_wizlist(victim, level);
            register_rank(victim->name,-1,-1,-1,-1);
	}
    }
    ...
    {
     victim->pcdata->security=0;
     if(IS_SET(victim->act,PLR_HOLYLIGHT))
      REMOVE_BIT(victim->act,PLR_HOLYLIGHT);
    }
    else
     victim->pcdata->security=victim->level-HERO;
    save_char_obj(victim);
    return;
}

/* Written by takeda (takeda@mathlab.sunysb.edu) */
/* extended and fixed by Gregor Stipicic, 2001 <gstipi@aon.at> */
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "    hunger drunk full quest pkp\n\r",		ch );
   ...
   if ( !str_cmp( arg2, "security" ) )	/* OLC */
    ...
    if ( !str_prefix( arg2, "pkp" ) )
    {
	if (IS_NPC(victim))
	{
	    send_to_char("Mobiles have no pk points.\n\r",ch);
	    return;
	}
        if( value < 1 || value > 7000)
        {
           send_to_char(" Pkp range is 1 to 7000.\n\r",ch);
           return;
        }
        victim->pcdata->pkp=value;
...
{
 CHAR_DATA *ch2;
 send_to_char(ch2->name,ch);
 return;
}

void do_arese(CHAR_DATA *ch, char *argument)
{
 send_to_char("If you want to ARESET, spell it out.\n\r",ch);
 return;
}
...
void do_check(CHAR_DATA *ch,char *argument)
{
 ...
 if(arg1[0]=='\0')
 {
  send_to_char("Check what? (groups|mp)\n\r",ch);
  return;
 }
 if(!strcmp(arg1,"groups"))
 {
  int class,group,warn=0,base,i,j,k,def,sub,glob=0,sn;
  for(class=0;class<MAX_CLASS;class++)
  {
   int cpdef;
   ...
   if((base=group_lookup(class_table[class].base_group))==-1)
   {
    sprintf(buf,"Couldn't find base group for class %s.\n\r",class_table[class].name);
    send_to_char(buf,ch);
    warn++;
    glob++;
    continue;
   }  
   if((def=group_lookup(class_table[class].default_group))==-1)
   {
    sprintf(buf,"Couldn't find default group for class %s.\n\r",class_table[class].name);
    send_to_char(buf,ch);
    warn++;
    glob++;
    continue;
   ...
   {
    if(group_table[base].spells[i]==NULL)
     break;
    for(j=0;j<MAX_IN_GROUP;j++)
    {
     if(group_table[def].spells[j]==NULL)
      break;
     if((sub=group_lookup(group_table[def].spells[j]))!=-1)
     {
      for(k=0;k<MAX_IN_GROUP;k++)
      {
       if(group_table[sub].spells[k]==NULL)
        break;
       if(!strcmp(group_table[sub].spells[k],group_table[base].spells[i]))
       {
        sprintf(buf,"%s: %s in base group is also member of group %s in default group.\n\r",class_table[class].name,group_table[base].spells[i],group_table[def].spells[j]);
        send_to_char(buf,ch);
        warn++;
        glob++;
       }
     ...
     else if(!strcmp(group_table[base].spells[i],group_table[def].spells[j]))
     {
      sprintf(buf,"%s: %s in base group is also member of default group.\n\r",
         class_table[class].name,
         group_table[base].spells[i]);
      send_to_char(buf,ch);
      warn++;
      glob++;
     } 
   ...
   for(group=0;group<MAX_GROUP;group++)
   {
    ...
    sn=nouse=count=0;
    if(group_table[group].spells[0]==NULL)
      break;
    for(i=0;i<MAX_CLASS;i++)
    { 
     if((i!=class) &&(!strcmp(group_table[group].name,class_table[i].base_group)
     || !strcmp(group_table[group].name,class_table[i].default_group)))
     sn=1;
    }
    if(sn)
     continue;
    if(!strcmp(group_table[group].name,class_table[class].default_group))
     isdef=TRUE;
    for(i=0;i<MAX_IN_GROUP;i++)
    {
     if(group_table[group].spells[i]==NULL)
      break;
     count++;
     if((sn=skill_lookup(group_table[group].spells[i]))!=-1)
     {
      if((skill_table[sn].skill_level[class]<=101)
           && (group_table[group].rating[class]<0))
      {
       sprintf(buf,"%s: %s in group %s cannot be gained.\n\r",
          class_table[class].name,
          group_table[group].spells[i],
      ...
      if(skill_table[sn].skill_level[class]>101)
        nouse++;
      if(isdef)
       cpdef+=skill_table[sn].rating[class];
     }
     else if((sub=group_lookup(group_table[group].spells[i]))!=-1)
     {
      if(group_table[sub].rating[class]<0)
      {
       sprintf(buf,"%s: subgroup %s in group %s cannot be gained.\n\r",
          class_table[class].name,
          group_table[sub].name,
          group_table[group].name);
       send_to_char(buf,ch);
       warn++;
       glob++;
      ...
      if(isdef)
       cpdef+=group_table[sub].rating[class];
     }
    }
    if((group_table[group].rating[class]>0)&&(count==nouse))
    {
     sprintf(buf,"%s: group %s can be gained although it contains no skills for this class.\n\r",
        class_table[class].name,
        group_table[group].name);
     send_to_char(buf,ch);
     warn++;
    ...
    if(isdef && (cpdef!=40))
    {
     sprintf(buf,"%s: default group cp: %d\n\r",
         class_table[class].name, cpdef);
     send_to_char(buf,ch);
     warn++;
     glob++;
    }
  ...
  if(warn)
  {
   sprintf(buf,"---Classes: %d warnings.\n\r",warn);
   send_to_char(buf,ch);
  }
  warn=0;
  for(group=0;group<MAX_GROUP;group++)
  {
    ...
    for(i=0;i<MAX_IN_GROUP;i++)
    {
     if(group_table[group].spells[i]==NULL)
      break;
     if((group_lookup(group_table[group].spells[i])==-1)
        && (skill_lookup(group_table[group].spells[i])==-1))
     {
      sprintf(buf,"%s in group %s is no skill, spell or group.\n\r",
        group_table[group].spells[i],
        group_table[group].name);
      send_to_char(buf,ch);
      warn++;
  ...
  if(warn)
  {
   sprintf(buf,"---Groups: %d warnings.\n\r",warn);
   send_to_char(buf,ch);
  }
  sprintf(buf,"Finished. %d warnings.\n\r",glob);
  send_to_char(buf,ch);
 }
 else if(!strcmp(arg1,"mp"))
 {
  ...
  if(arg2[0]=='\0')
  {
   send_to_char("Check what concerning mp violations? (smult)\n\r",ch);
   return;
  }
  if(!strcmp(arg2,"smult"))
  {
   int found=0;
   ...
   { 
    ...
    if(in_socket>1)
    {
     int cnt=0;
     found++;
     sprintf(buf,"%s: ",pSocket->socket);
     for(pChar=pSocket->socket_char_first;pChar;pChar=pChar->next)
     {
      cnt++;
      if(cnt==in_socket)
       sprintf(buf,"%s%s\n\r",buf,pChar->character);
      else
       sprintf(buf,"%s%s ",buf,pChar->character);
     }
     send_to_char(buf,ch);   
    }
   }
   sprintf(buf,"Sockets with multiple characters: %d\n\r",found);
   send_to_char(buf,ch);
  }
  else
   do_check(ch,"mp");  
...
void do_addallow(CHAR_DATA *ch,char *argument)
{
 char arg1[MAX_INPUT_LENGTH];
 char arg2[MAX_INPUT_LENGTH];
 char arg3[MAX_INPUT_LENGTH];
 ALLOW_DATA *pAllow=NULL;
 char buf[MAX_STRING_LENGTH];
 int count,i;
 char types[ALLOW_MAX][10]=
    { "prefix","suffix","exact","all" };
 ...
 if(arg1[0]=='\0')
 {
  send_to_char("Syntax: addallow <name> <mask> <prefix|suffix|exact|all>\n\r",ch);
  send_to_char("        addallow <name>\n\r",ch);
  send_to_char("	addallow del <name> <number>\n\r",ch);
  send_to_char("	addallow all\n\r",ch);
  return;
 }
 if(!strcmp(arg1,"del"))
 {
  ALLOW_DATA *previous_data=NULL;
  if((arg2[0]=='\0') || (arg3[0]=='\0'))
  {
   send_to_char("Syntax: addallow del <name> <number>\n\r",ch);
   send_to_char("(Type 'addallow <name>' to find out about <number>.)\n\r",ch);
   return;
  }
  i=atoi(arg3);
  if(i<1)
  {
   send_to_char("Illegal number.\n\r",ch);
   return;
  }
  count=0;
  i--;
  ...
  {
   if(!strcmp(capitalize(arg2),pAllow->name))
   {
    if(i==count++)
    {
     if(pAllow==allow_list)
      allow_list=pAllow->next;
     else
      previous_data->next=pAllow->next;
     send_to_char("{cAllow deleted.{x\n\r",ch);
     save_allows();
     return;
    }
   }
   previous_data=pAllow;
  }  
  if(!count)
   send_to_char("{cNo allow for this name.{x\n\r",ch);
  else
   send_to_char("{xNo allow under that number.{x\n\r",ch);
  return;
 }
 if(!strcmp(arg1,"all"))
 {
  count=0;
  sprintf(buf,"{c%-15s  %-40s  %-8s{x\n\r",
      "Name","Mask","Type");
  send_to_char(buf,ch);
  for(pAllow=allow_list;pAllow;pAllow=pAllow->next)
  {
   sprintf(buf,"{m%-15s{x  %-40s  %-8s\n\r",
     pAllow->name,pAllow->mask,types[pAllow->type]);
   send_to_char(buf,ch);
   count++;
  }
  sprintf(buf,"{D%d{c total allows.{x\n\r",count);
  send_to_char(buf,ch);
  return;
 }
 else if(arg2[0]=='\0')
 {
  count=1;
  sprintf(buf,"{c%-3s %-15s  %-40s  %-8s{x\n\r",
   "No.","Name","Mask","Type");
  send_to_char(buf,ch);
  for(pAllow=allow_list;pAllow;pAllow=pAllow->next)
   if(!strcmp(pAllow->name,capitalize(arg1)))
   {
    sprintf(buf,"{D%2d{c) {m%-15s{x  %-40s  %-8s\n\r",
      count++,pAllow->name,pAllow->mask,types[pAllow->type]);
    send_to_char(buf,ch);
   }
  sprintf(buf,"{D%d{c allows found.{x\n\r",count-1);
  send_to_char(buf,ch);
  return;
 }
 else
 {
  ...
  for(i=0;i<ALLOW_MAX;i++)
  {
   if(!strcmp(types[i],arg3))
   {
    add_allow(arg1,arg2,i);
    send_to_char("{cAllow added.{x\n\r",ch);
    save_allows();
    return;
   } 
  }
...
void do_confiscate(CHAR_DATA *ch,char *argument)
{
 ...
 printf_log("%s confiscates %s's %s at room %d.",
    ch->name,victim->name,obj->short_descr,ch->in_room->vnum);
} 
