...
void do_unread(CHAR_DATA *ch)
{
    ...
    if ((count = count_spool(ch,claninfo_list)) > 0)
    {
	found = TRUE;
	sprintf(buf,"{cThere %s %d new {Mc{mlaninfo%s{c waiting.{x\n\r",
    ...
    {
	found = TRUE;
	sprintf(buf,"{cThere %s %d new {Mn{mews{c article%s waiting.{x\n\r",
	    count > 1 ? "are" : "is",count, count > 1 ? "s" : "");
	send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"{cThere %s %d {Rc{rhange%s{c waiting to be read.{x\n\r",
	    count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
        send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"{cYou have %d new {Mn{mote%s{c waiting.{x\n\r",
	    count, count > 1 ? "s" : "");
	send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"{cYou have %d unread {Mi{mdea%s{c to peruse.{x\n\r",
	    count, count > 1 ? "s" : "");
	send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"{c%d %s been added.{x\n\r",
	    count, count > 1 ? "{Mp{menalties{c have" : "{Mp{menalty{c has");
	send_to_char(buf,ch);
    }

    if (!found)
	send_to_char("{cYou have no unread notes.{x\n\r",ch);
}
...
{
    parse_note(ch,argument,NOTE_CLANINFO);
}
...
void load_notes(void)
{
    ...
    strcat(boot_buf,"ves\n\r  the a");
    load_thread(CLANINFO_FILE,&claninfo_list, NOTE_CLANINFO, 0);
...
bool hide_note (CHAR_DATA *ch, NOTE_DATA *pnote)
{
	...
	case NOTE_CHANGES:
	    ...
	    break;
	case NOTE_CLANINFO:
	    last_read = ch->pcdata->last_claninfo;
	    break;
    }
...
void update_read(CHAR_DATA *ch, NOTE_DATA *pnote)
{
    ...
    switch (pnote->type)
    {
	    ...
	    ch->pcdata->last_changes = UMAX(ch->pcdata->last_changes,stamp);
            break;
	case NOTE_CLANINFO:
	    ch->pcdata->last_claninfo = UMAX(ch->pcdata->last_claninfo,stamp);
	    break;
    }
}
...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
    ...
    if ( arg[0] == '\0' || !str_prefix( arg, "read" ) )
    {
        ...
        /* read next unread note */
        {
            ...
            for ( pnote = *list; pnote != NULL; pnote = pnote->next)
            {
                if (!hide_note(ch,pnote))
                {
                    sprintf( buf, "{C[{c%3d{C]{x {c%s{D:{x %s\n\r{cDate{D: {x%s\n\r{cTo{D:{x %s\n\r",
                        vnum,
                        pnote->sender,
                        pnote->subject,
                        pnote->date,
        ...
        for ( pnote = *list; pnote != NULL; pnote = pnote->next )
        {
            ...
            {
                sprintf( buf, "{C[{c%3d{C]{c %s{D:{x %s\n\r{cDate{D: {x%s\n\r{cTo{D:{x %s\n\r",
                    vnum - 1,
                    pnote->sender,
                    pnote->subject,
                    pnote->date,
	...
	for ( pnote = *list; pnote != NULL; pnote = pnote->next )
	{
	    if ( is_note_to( ch, pnote ) )
	    {
		sprintf( buf, "{C[{c%3d%s{C]{c %s{D:{x %s\n\r",
	...
	if (!vnum)
	{
		...
		case NOTE_CHANGES:
		    ...
		    break;
		case NOTE_CLANINFO:
		    sprintf( buf, "There are no claninfos for you.\n\r");
		    break;
	    }
	    add_buf(output,buf);
	}
	...
	switch(type)
	{
		...
		break;
	    case NOTE_CLANINFO:
		ch->pcdata->last_claninfo = current_time;
		break;
	}
	return;
    }
    ...
    ||  (type == NOTE_CLANINFO && !is_clead(ch) && !IS_IMMORTAL(ch)))
    {
