...
AUCTION_DATA		auction_info[MAX_AUCTION];
int			chain;
int               reboot_counter;

sh_int			auction_number=0;
sh_int			auction_ticket=0;
...
sh_int			gsn_chaos_style;
sh_int			gsn_elemental_style;
sh_int			gsn_offhand;
...
sh_int			gsn_ultravision;
sh_int			gsn_protective_force;
...
sh_int			gsn_strike;
sh_int			gsn_slip;
sh_int			gsn_advanced_disarm;
sh_int			gsn_forage;
sh_int			gsn_whirlwind;
sh_int			gsn_blade_poison;
sh_int			gsn_shield;
sh_int			gsn_divine_blessing;
sh_int			gsn_infernal_offering;
sh_int			gsn_cascade;
sh_int			gsn_essence_of_mist;
sh_int			gsn_essence_of_bat;
sh_int			gsn_essence_of_wolf;
sh_int			gsn_ambush;
...
int			newmobs = 0;
int			newobjs = 0;
sh_int			arena = FIGHT_OPEN;
sh_int			game_state = GS_CHAOS;
bool			gs_changing = TRUE;
...
/* OLC */     else if ( !str_cmp( word, "AREADATA" ) ) new_load_area(fpArea);
	...
	load_songs();
    log_string( "Loading SDB (Socket Database)." );
        load_sockets();
    log_string( "Loading Allows." );
        load_allows();
    log_string( "Loading recent Pks.");
	load_pkills();
    log_string( "Loading rankings." );
	load_ranks();
    }
...
void clone_mobile(CHAR_DATA *parent, CHAR_DATA *clone)
{
	...
	clone->mod_stat[i]	= parent->mod_stat[i];
        clone->pmod_stat[i]	= parent->pmod_stat[i];
    }
...
/*
 * Translates mob virtual number to its room index struct.
 * Only returns a room index when room is in linked area.
 * Hash table lookup.
 */
ROOM_INDEX_DATA *get_room_index_safe( int vnum )
{
    ...
    for ( pRoomIndex  = room_index_hash[vnum % MAX_KEY_HASH];
	  ...
	  pRoomIndex  = pRoomIndex->next )
    {
	if ( pRoomIndex->vnum == vnum && (!(strstr(pRoomIndex->area->builders,"Unlinked"))))
	    return pRoomIndex;
    }
...
void printf_log( char *fmt, ...)
{
 ...
 if((fp=fopen(LOG_FILE,"a"))==NULL)
  bug("Error in write_log, couldn't open logfile",0);
 strtime = ctime( &current_time );
 strtime[strlen(strtime)-1] = '\0';
 fprintf( fp, "%s :: %s\n", strtime, buf);
 fclose(fp);
...
void randomize_entrances( int code )
{
    ...
    if (/*(code == 0) || (code == ROOM_VNUM_CLANS)*/0)
    {
