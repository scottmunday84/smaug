...
typedef struct	socket_char_data	SOCKET_CHAR_DATA;
typedef struct	allow_data		ALLOW_DATA;
...
#define	OWNER			MAX_LEVEL
#define	IMPLEMENTOR		(MAX_LEVEL - 1)
#define	ENFORCER		(MAX_LEVEL - 2)
#define	LDB			(MAX_LEVEL - 3)
#define	HQI			(MAX_LEVEL - 4)
#define	QCB			(MAX_LEVEL - 5)
#define BUILDER			(MAX_LEVEL - 6)
#define	QIM			(MAX_LEVEL - 7)
#define TRAINEE			(MAX_LEVEL - 8)
...
#define	ALLOW_PREFIX	0	
#define	ALLOW_SUFFIX	1
#define	ALLOW_EXACT	2
#define ALLOW_ALL	3
#define ALLOW_MAX	4
...
#define	MAX_IN_SOCKET	1024

struct socket_data
{
  SOCKET_DATA *next;
  char *socket;
  SOCKET_CHAR_DATA *socket_char_first;
};
 
struct socket_char_data
{
 SOCKET_CHAR_DATA *next; 
 char *character;
}; 
...
#define OBJ_VNUM_SURVIVAL_K	   3162 /*remove that one*/
...
#define WEAPON_ROTTING		(I)
#define WEAPON_LIGHT		(J)
#define	WEAPON_DROWNING		(K)
#define	WEAPON_UNHOLY		(L)

/* stances */
#define	STANCE_NONE		0
#define	STANCE_FLAMING		1
#define	STANCE_FROST		2
#define	STANCE_VAMPIRIC		3
#define	STANCE_SHOCKING		4
#define	STANCE_POISON		5
...
#define APPLY_STR_P		     27
#define APPLY_WIS_P		     28
#define APPLY_INT_P                  29
#define APPLY_DEX_P                  30
#define APPLY_CON_P                  31	
...
#define ROOM_VNUM_TEMPLE	   20100
#define ROOM_VNUM_TEMPLEB	   20100
#define ROOM_VNUM_PIT		   20124
#define ROOM_VNUM_ALTAR		   20101 
#define ROOM_VNUM_ALTARB	   20101
...
extern 	sh_int	gsn_4th_dual;
extern  sh_int  gsn_critical_hit;
extern  sh_int  gsn_tiger_style;
extern  sh_int  gsn_dragon_style;
extern  sh_int  gsn_chaos_style;
extern  sh_int  gsn_elemental_style;
extern  sh_int  gsn_offhand;
...
extern	sh_int	gsn_ultravision;
extern	sh_int	gsn_protective_force;
...
extern	sh_int	gsn_advanced_disarm;
extern	sh_int	gsn_forage;
extern	sh_int	gsn_whirlwind;
extern 	sh_int	gsn_blade_poison;
extern  sh_int	gsn_shield;
extern 	sh_int	gsn_divine_blessing;
extern	sh_int	gsn_infernal_offering;
extern	sh_int	gsn_cascade;
extern	sh_int	gsn_essence_of_mist;
extern 	sh_int	gsn_essence_of_bat;
extern	sh_int	gsn_essence_of_wolf;
...
#define FLEE_DAZE(ch, daze)	((ch)->flee_daze = UMAX((ch)->flee_daze, (daze)))
...
#define PERS(ch, looker)	( can_see( looker, (ch) ) ?\
                                ...
                                (((ch)->level>101) ?\
                                "{RA{rn {RI{rm{Rm{ro{Rr{rt{Ra{rl{x"\
                                : "someone")\
                                : "someone"))
#define SWPERS(ch, looker)	( can_see( looker, (ch) ) ?\
				( IS_NPC(ch) ? (ch)->short_descr\
				: (ch)->name ) : (IS_IMMORTAL(ch) ?\
				(((ch)->level>101) ?\
				(IS_SHIELDED(ch,SHD_SWALK) ?\
                                "{RA{rn {RI{rm{Rm{ro{Rr{rt{Ra{rl {c({CSW{c){x"\
                                : "{RA{rn {RI{rm{Rm{ro{Rr{rt{Ra{rl{x")\
                                : (IS_SHIELDED(ch,SHD_SWALK) ?\
                                "Someone {c({CSW{c){x" : "Someone"))\
                                :(IS_SHIELDED(ch,SHD_SWALK) ?\
                                "Someone {c({CSW{c){x" : "Someone")))
...
extern		int			quest_flags;
extern		SOCKET_DATA	*	socket_first;
...
#define LOG_FILE	"../log/log.txt"	/* log	*/
#define SOCKET_FILE	"../log/socket.txt"	/* database for add_socket */
#define	ALLOW_FILE	"allow.txt"		/* for immortal site checks */
#define HELP_FILE	"../log/help.txt"	/* for tracking requested but
						   unavailable helpfiles */
#define PKILL_FILE	"../log/pkill.txt"	/* for storing recent pks */
#define RANK_FILE	"../log/rank.txt"  	/* for storing ranking DB */
#endif
...
#define CLANINFO_FILE	"clani.not"
...
void 	rotting_effect	args( (void *vo, int level, int dam, int target) );
void 	light_effect	args( (void *vo, int level, int dam, int target) );
void	drowning_effect	args( (void *vo, int level, int dam, int target) );
void  	unholy_effect	args( (void *vo, int level, int dam, int target) );
...
int	xdamage		args( ( CHAR_DATA *ch, CHAR_DATA *victim, int dam,
				int dt, int class, bool show, int suppress));
...
int 	check_immune	args( (CHAR_DATA *ch, int dam_type, int dt) );
...
extern int port,control,wwwport,wwwcontrol;

void 	comm_arena	args( (CHAR_DATA *ch, CHAR_DATA *victim, char *argument));
...
#define EXBIT1_CHALLENGER	(A)
#define EXBIT1_CHALLENGED	(B)
#define	PLR_QUESTOR		(C)

void	set_killed_flag	args( (CHAR_DATA *ch) );
bool  	has_killed_flag	args( (CHAR_DATA *ch) );
void 	remove_killed_flag args( (CHAR_DATA *ch) );
bool	in_range	args( (CHAR_DATA *ch, CHAR_DATA *victim));
bool	has_pkflag	args( (CHAR_DATA *ch) );
bool	can_empower	args( (char *name) );
...
extern	AUCTION_DATA	auction_info[MAX_AUCTION];
extern  sh_int		auction_number;
extern	sh_int		auction_ticket;

#define QUEST_OFF	0
#define	QUEST_ON	1
#define	QUEST_CLOSED	2

extern	sh_int		game_state;
extern 	bool		gs_changing;

void printf_log args((char *fmt, ...)) __attribute__ ((format(printf,1,2)));
void reset_arena args((void));
bool can_quit args((CHAR_DATA *ch));
void load_sockets args((void));
void save_sockets args((void));
void add_socket args((char *socket, char *name));
void load_allows args((void));
void save_allows args((void));
void add_allow args((char *name, char *mask, int type));
bool is_allowed args((char *name,char *socket));

extern	ALLOW_DATA *allow_list;
extern 	sh_int	conn_so_far;
extern	sh_int	max_on_ever;
extern	PKILL_DATA *pkill_list;	
extern	RANK_DATA *rank_list;
extern  TEAM_DATA *team_list;

void save_pkills args((void));
void load_pkills args((void));
void register_pk args((char *killer, char *victim, int gs, int pkp, int killers));
...
void register_rank args((char *name, int pkp, int race, int class, int clan));

bool is_tlead args((CHAR_DATA *ch));
bool is_same_team args((CHAR_DATA *ch,CHAR_DATA *victim));
TEAM_DATA *get_team args((CHAR_DATA *ch));

#define QUEST_COMBAT	(A)
#define QUEST_NOAGG	(B)
#define QUEST_NORECALL	(C)
#define	QUEST_NOGATE	(D)
