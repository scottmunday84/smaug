...
int check_immune(CHAR_DATA *ch, int dam_type, int dt)
{
    int immune, def;
    int bit;
    ...
    if ((dt >= TYPE_HIT) && (dt <= (TYPE_HIT+MAX_DAMAGE_MESSAGE))
      && (dt!=(TYPE_HIT+17)) && ((dt<(TYPE_HIT+40))
            || (dt>(TYPE_HIT+60)))) /* regular weapon attack
                                       and not sensai damage */
    {
    ...
    else if(skill_table[dt].spell_fun!=spell_null)/* magical attack */
    {	
	...
	case(DAM_SOUND):	bit = IMM_SOUND;	break;
        case(DAM_EARTH):	bit = IMM_EARTH;	break;
...
bool class_can_use(CHAR_DATA *ch, OBJ_DATA *obj)
{
    if (!is_class_obj(obj))
	return TRUE;
    if(obj->class>(MAX_CLASS/2))
     return 
           ((ch->class==obj->class)
           || ((ch->class) == (obj->class-(MAX_CLASS/2))));
    else
     return 
          ((ch->class==obj->class)
           || ((ch->class) == (obj->class+(MAX_CLASS/2))));
...
int get_skill(CHAR_DATA *ch, int sn)
{
	...
	|| IS_SET(ch->act,ACT_RANGER) || IS_SET(ch->act,ACT_WARRIOR)))
	    skill = 20 + ch->level;
             ...
             || IS_SET(ch->off_flags, OFF_CIRCLE)))
	    skill = 10 + ch->level;

        else if (sn == gsn_feed && (IS_SET(ch->act,ACT_VAMPIRE) 
             || IS_SET(ch->off_flags, OFF_FEED)))
            skill = 10 + ch->level;
...
void reset_char(CHAR_DATA *ch)
{
    ...
    for (af = ch->affected; af != NULL; af = af->next)
    {
        ...
        switch(af->location)
        {
                ...
                case APPLY_HIT:         ch->max_hit             += mod; break;
                case APPLY_MOVE:        ch->max_move            += mod; break;
                case APPLY_STR_P:       ch->pmod_stat[STAT_STR] += mod; break;
                case APPLY_DEX_P:       ch->pmod_stat[STAT_DEX] += mod; break;
                case APPLY_INT_P:       ch->pmod_stat[STAT_INT] += mod; break;
                case APPLY_WIS_P:       ch->pmod_stat[STAT_WIS] += mod; break;
                case APPLY_CON_P:       ch->pmod_stat[STAT_CON] += mod; break;
...
int get_curr_stat( CHAR_DATA *ch, int stat )
{
	...
	max = pc_race_table[ch->race].max_stats[stat] + 4 
                + ch->pmod_stat[stat];
        /* pmod_stat stores temporary increases of the
           racial stat max (used in necro forms, etc.) */
    ...
    if((ch->perm_stat[stat]+ch->mod_stat[stat])>max)
     return max;
    if((ch->perm_stat[stat]+ch->mod_stat[stat])<0)
     return 0;
    return (ch->perm_stat[stat]+ch->mod_stat[stat]);
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
    ...
    case APPLY_SIZE: if(fAdd) ch->size = mod;
                     else 
                     {
                      if(IS_NPC(ch))
...
void affect_join( CHAR_DATA *ch, AFFECT_DATA *paf )
{
	...
	if ( paf_old->type == paf->type )
	{
	    ...
	    if((paf_old->duration>=1000)&&(paf->duration>=1000))
             paf->duration += (paf_old->duration-1000);
            else if((paf_old->duration>=1000)&&(paf->duration<1000))
             ;
            else if((paf_old->duration<1000)&&(paf->duration>=1000))
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    {
	bug( "Equip_char: already equipped (%d).", obj->pIndexData->vnum );
	return;
    }
	...
	/*if ( paf->location == APPLY_SPELL_AFFECT )
	    ...
	    affect_modify( ch, paf, TRUE );*/
         /* new code by Gregor Stipicic, 2001 */
         if(paf->location==APPLY_SPELL_AFFECT && (!is_affected(ch,paf->type)))
         {
            (*skill_table[paf->type].spell_fun)
               (paf->type,paf->level,ch,ch,TARGET_CHAR);
            chaff=affect_find(ch->affected,paf->type);
            if(chaff)
             chaff->duration=-1;
         }
         else
	    ...
	    if ( paf->location == APPLY_SPELL_AFFECT && is_affected(ch,paf->type))
	    {
	       /* for ( lpaf = ch->affected; lpaf != NULL; lpaf = lpaf_next )
			...
			lpaf_next = NULL;
               ...
               /* new code by Gregor Stipicic, 2001 */
               chaff=affect_find(ch->affected,paf->type);
               if(chaff && (chaff->duration==-1) && (chaff->level==paf->level))
               {
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    /* check whether ch is on victim's view */
    if(!IS_NPC(victim) && !IS_NPC(ch))
    {
     for(pos=0;pos<MAX_VIEW;pos++)
     {
      if(victim->pcdata->view[pos]==NULL)
       break;
      if(!strcasecmp(victim->pcdata->view[pos],ch->name))
      {
       fView = TRUE;
       break;
      }
    ...
    if ( IS_SHIELDED(victim, SHD_SWALK) 
     && !is_affected(ch, gsn_ultravision)
         && !is_same_clan(ch,victim)
         && !fView
         && !(IS_NPC(ch) && (number_percent()<(ch->level/2+25))))
         return FALSE;
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_STR_P:		return "str (max)";
    case APPLY_DEX_P:		return "dex (max)";
    case APPLY_INT_P:		return "int (max)";
    case APPLY_WIS_P:		return "wis (max)";
    case APPLY_CON_P:           return "con (max)";
...
char *shield_bit_name( int vector )
{
    ...
    if ( vector & SHD_SHADOW		) strcat( buf, " shadow"	);
    if ( vector & SHD_BLADE		) strcat( buf, " blade"		);
    if ( vector & SHD_PRISMATIC		) strcat( buf, " prismatic"	);
    if ( vector & SHD_EARTH		) strcat( buf, " earth"		);
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *comm_bit_name(int comm_flags)
{
    ...
    if (comm_flags & COMM_NOARENA	) strcat(buf, " noarena");    
...
char *suppress_bit_name(int suppress_flags)
{
  static char buf[512];
...
char *weapon_bit_name(int weapon_flags)
{
    ...
    if (weapon_flags & WEAPON_ROTTING	) strcat(buf, " rotting");
    if (weapon_flags & WEAPON_LIGHT	) strcat(buf, " light");
