...
void do_aedit( CHAR_DATA *ch, char *argument )
{
	...
	{
	    if (!IS_NPC(ch) && !IS_TRUSTED(ch,LDB) )
		{
	    ...
	    if (get_area_data(value) != NULL)
	       {
	        	send_to_char("This area is already existing!",ch);
	        	return;
	       }
	    pArea               =   new_area();
    ...
    if (IS_SWITCHED(ch) || (ch->pcdata->security<pArea->security))
    {
...
void do_oedit( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( arg1, "create" ) )
	{
	    ...
	    if ( !IS_BUILDER( ch, pArea ) )
	    {
		send_to_char( "Insufficent security to create object.\n\r" , ch );
	        return;
	    }
...
void do_medit( CHAR_DATA *ch, char *argument )
{
	...
	if ( !str_cmp( arg1, "create" ) )
	{
	    ...
	    if ( !IS_BUILDER( ch, pArea ) )
	    {
		send_to_char( "Insufficient security to create mobs.\n\r" , ch );
	        return;
	    }
